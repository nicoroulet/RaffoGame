...
void menu::add_button(int x, int y, ALLEGRO_BITMAP * img, char const * text, ALLEGRO_COLOR color) {
	ALLEGRO_BITMAP * aux = al_get_target_bitmap();
	al_set_target_bitmap(box);
	al_draw_bitmap(img, x, y, 0);
	if (!font) {
		std::cerr << "Menu ERROR (add_button): fuente indefinida\n";
		return;
	}
	...
	al_draw_text(font, color, x + al_get_bitmap_width(img) / 2, y + (al_get_bitmap_height(img)) / 2 - fontsize, ALLEGRO_ALIGN_CENTRE, text);
...
void menu::set_font(char const * filename, int size) {
	...
	if (!font) std::cerr << "Menu ERROR (set font): cargando fuente\n";
	fontsize = size;
}
